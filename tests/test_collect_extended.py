...
class TestSaveAndLoadHashFile:
    def test_roundtrip(self, tmp_path):
        ...
        collect_mod.save_hash_file(data, hashfile)
        assert os.path.exists(hashfile)

        loaded = collect_mod.load_hash_file(hashfile)
        assert len(loaded) == 1
    ...
    def test_load_nonexistent_file(self, tmp_path):
        result = collect_mod.load_hash_file(str(tmp_path / "nonexistent.txt"))
        assert result == []

    def test_save_empty_list(self, tmp_path):
        hashfile = str(tmp_path / "empty.txt")
        collect_mod.save_hash_file([], hashfile)
        assert os.path.exists(hashfile)
        loaded = collect_mod.load_hash_file(hashfile)
        assert loaded == []
...
class TestMoveToLabel:
    def test_move_files(self, tmp_path):
        ...
        try:
            collect_mod.move_to_label(str(tmp_path), True, files)  # keepolddata=True (copy)
...
class TestZifferDataFilesCollect:
    def test_finds_nested_files(self, tmp_path):
        ...
        files = collect_mod.ziffer_data_files(str(tmp_path))
        assert len(files) == 3
